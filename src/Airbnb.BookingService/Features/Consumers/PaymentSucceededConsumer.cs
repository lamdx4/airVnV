    ...
    BookingDbContext dbContext) : IConsumer<PaymentSucceededEvent>
{
    public async Task Consume(ConsumeContext<PaymentSucceededEvent> context)
    {
        ...
        // Idempotency: already in terminal/expected state → skip (stale/duplicate message)
        if (booking.Status == BookingStatus.Confirmed || booking.Status == BookingStatus.AwaitingApproval)
        {
            logger.LogInformation("Booking {BookingId} already in status {Status}, skipping PaymentSucceeded.", message.BookingId, booking.Status);
            return;
        }

        if (booking.Status != BookingStatus.Pending)
        {
            logger.LogWarning("Booking {BookingId} is in unexpected status {Status} for PaymentSucceeded, skipping.", message.BookingId, booking.Status);
            return;
        }
