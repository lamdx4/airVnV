...
namespace Airbnb.UserService.Features.Admin.UserManagement.RejectKyc;

public record Request(Guid UserId, string Reason) : Mediator.ICommand<Response>;
...
public sealed class Handler(Airbnb.UserService.Infrastructure.UserDbContext db) : ICommandHandler<Request, Response>
{
    public async ValueTask<Response> Handle(Request req, CancellationToken ct)
    {
        ...
        user.RejectKyc(req.Reason);
        await db.SaveChangesAsync(ct);

        return new Response(user.Id, "KycRejected", DateTimeOffset.UtcNow, adminId);
    }
}
...
public sealed class Endpoint(IMediator mediator) : FastEndpoints.Endpoint<Request, ApiResponse<Response>>
{
    public override void Configure()
    {
        ...
        Policies("AdminOnly");
        Summary(s => {
            s.Summary = "Admin reject user KYC (identity verification)";
            s.Description = "Possible Error Codes: \n" +
                            "- **NOT_FOUND**: User not found.\n" +
                            "- **KYC_NOT_PENDING**: Only pending KYC can be rejected.\n" +
                            "- **UNAUTHORIZED**: Not authorized to access this endpoint.";
            s.Responses[200] = "KYC rejected successfully.";
