    ...
    : FastEndpoints.Endpoint<FastEndpoints.EmptyRequest, ApiResponse<PayoutActionResponse>>
{
    ...
    {
        Patch("/api/payouts/admin/{PayoutId}/cancel");
        AllowAnonymous();
        Summary(s =>
        {
            s.Summary = "Admin: cancel a pending or failed payout";
            s.Description = "Transitions a Pending or Failed payout to Cancelled status.";
            s.Responses[200] = "Payout cancelled.";
