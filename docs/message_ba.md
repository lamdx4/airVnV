...
- **Nghiệp vụ:** Khi đang mở ứng dụng, tin nhắn nhảy real-time không cần f5. Khi user bấm vào một Conversation, tất cả tin nhắn trong đó chuyển thành "Đã đọc".
- **Developer cần làm:**
  - Xây dựng **SignalR Hub** (`ChatHub`) để đẩy dữ liệu real-time. Do ứng dụng là Microservices chạy nhiều instance, **bắt buộc phải cắm Redis Backplane** vào SignalR để các server đồng bộ connection với nhau.
  - Viết Endpoint `PATCH /api/chat/conversations/{id}/read` để update trạng thái `IsRead = true` cho toàn bộ messages của người kia gửi.
...
## 3. THIẾT KẾ CƠ SỞ DỮ LIỆU DỰ KIẾN (DATABASE SCHEMA)

Để người mới dễ hình dung, Service này sẽ có một database riêng (`chatdb`) trong PostgreSQL.

**Table `ChatUsers` (Data Replication)**

- `Id` (PK - Guid) -> Mapping với `UserId` bên UserService
- `FirstName` (String)
- `LastName` (String)
- `AvatarUrl` (String?)
*Bảng này được sinh ra và cập nhật thông qua Event `UserRegisteredEvent` và `UserProfileUpdatedEvent` từ RabbitMQ.*

**Table `Conversations`**

- `Id` (PK - Guid)
- `Participant1Id` (Guid) -> ID của Guest
- `Participant2Id` (Guid) -> ID của Host
- `PropertyId` (Guid) -> Căn nhà mà họ đang chat về
- `BookingId` (Guid?) -> Null nếu chỉ là hỏi han, có giá trị nếu đã thành chuyến đi.
- `LastMessageContent` (String) -> Cập nhật liên tục để hiển thị ngoài Inbox cho lẹ.
- `LastMessageAt` (DateTime) -> Để sort Inbox.
- `Status` (Enum: Active, Archived) -> Người dùng có thể ẩn/lưu trữ luồng chat cũ.
...
- `Type` (Enum: Text, Image, System) -> Loại tin nhắn
- `IsRead` (Bool) -> Đánh dấu đã xem hay chưa
- `CreatedAt` (DateTime)
