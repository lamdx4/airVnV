...
- **Mô tả:** Định nghĩa một tập hợp các thuật toán (Payment Providers), đóng gói từng thuật toán lại, và làm cho chúng có thể hoán đổi cho nhau. Handler sử dụng Interface để thực hiện thanh toán mà không cần biết nhà cung cấp cụ thể là ai.
- **Interface:** [IPaymentProvider.cs](file:///home/lamdx4/Projects/Airbnb/src/Airbnb.PaymentService/Infrastructure/PaymentGateways/IPaymentProvider.cs)
- **Implementations:** [VnpayProvider.cs](file:///home/lamdx4/Projects/Airbnb/src/Airbnb.PaymentService/Infrastructure/PaymentGateways/VnpayProvider.cs), `StripeProvider.cs` (Future).

### 20. Provider Resolver Pattern (Dynamic Strategy Selection)

- **Mô tả:** Lựa chọn Strategy phù hợp tại thời điểm runtime dựa trên dữ liệu cấu hình (Master Data từ PropertyService). Hệ thống tự động điều hướng: `CountryCode` -> `Active Gateway` -> `Provider Implementation`.
- **Áp dụng:** [PaymentProviderResolver.cs](file:///home/lamdx4/Projects/Airbnb/src/Airbnb.PaymentService/Infrastructure/PaymentGateways/PaymentProviderResolver.cs)
- **Source of Truth:** Bảng `PaymentGateways` trong `PropertyService`.

### 21. Idempotency Guard (DB-Level Filtered Index)

- **Mô tả:** Đảm bảo một yêu cầu thanh toán chỉ tạo ra duy nhất một phiên (session) đang hoạt động. Sử dụng Unique Filtered Index để chặn Race Condition ngay tại tầng Database thay vì chỉ kiểm tra bằng code.
- **Cấu hình DB:** [PaymentDbContext.cs](file:///home/lamdx4/Projects/Airbnb/src/Airbnb.PaymentService/Infrastructure/PaymentDbContext.cs) – `ix_payments_booking_pending`.
- **Logic:** [InitiatePayment/Handler.cs](file:///home/lamdx4/Projects/Airbnb/src/Airbnb.PaymentService/Features/InitiatePayment/Handler.cs) – catch `DbUpdateException` và xử lý hết hạn (Expiry).
